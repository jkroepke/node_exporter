...
// NewDiskstatsDeviceFilterFlags is register CLI flags
func NewDiskstatsDeviceFilterFlags(app *kingpin.Application) {
	diskstatsDeviceExclude = app.Flag(
	...
	oldDiskstatsDeviceExclude = app.Flag(
		"collector.diskstats.ignored-devices",
		"DEPRECATED: Use collector.diskstats.device-exclude",
	).Hidden().String()

	diskstatsDeviceInclude = app.Flag("collector.diskstats.device-include", "Regexp of diskstats devices to include (mutually exclusive to device-exclude).").String()
}
